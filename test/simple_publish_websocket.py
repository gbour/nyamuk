...
port = 8081 if ssl else 8080
client = Nyamuk("test_nyamuk", server="test.mosquitto.org", port=port, websocket=True, ssl=ssl)
...
client.publish('foo/bar', 'this is a test ({0})'.format(('ssl' if ssl else 'plain')) , qos=1)
